...
export const api = {
  ...
  async getSiteNotes(siteId) { return useMock ? delay(bySite(mockData.notes, siteId)) : request(`/site-notes/search${query({ q: '', site_id: Number(siteId), limit: 5 })}`); },
  ...
  async getSiteWorkspace(id) {
    ...
    const graph = await this.getCausalGraph(risks[0]?.id);
